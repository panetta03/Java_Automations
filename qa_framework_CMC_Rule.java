package ap_unix.qa_framework_process_cmc_0_1;
...
/**
 * Job: QA_Framework_Process_CMC Purpose: CSV File<br>
 * Description: Monthly <br>
 * @author user@talend.com
 * @version 6.1.1.20151214_1327
 * @status DEV
 */
public class QA_Framework_Process_CMC implements TalendJob {
	static {
		System.setProperty("TalendJob.log", "QA_Framework_Process_CMC.log");
	}
	private static org.apache.log4j.Logger log = org.apache.log4j.Logger
			.getLogger(QA_Framework_Process_CMC.class);
	...
	public class ContextProperties extends java.util.Properties {
		...
		public void synchronizeContext() {
			...
			if (successMessage != null) {
				...
				this.setProperty("successMessage", successMessage.toString());
	...
	private final String jobName = "QA_Framework_Process_CMC";
		...
		public void printStackTrace() {
			...
			if (!(e instanceof TDieException)) {
				if (e instanceof TalendException) {
					...
					QA_Framework_Process_CMC.this.exception = e;
				}
			}
			if (!(e instanceof TalendException)) {
				try {
					for (java.lang.reflect.Method m : this.getClass()
						...
						if (m.getName().compareTo(currentComponent + "_error") == 0) {
							m.invoke(QA_Framework_Process_CMC.this,
	...
	public static class row6Struct implements
		...
		final static byte[] commonByteArrayLock_AP_UNIX_QA_Framework_Process_CMC = new byte[0];
		static byte[] commonByteArray_AP_UNIX_QA_Framework_Process_CMC = new byte[0];
		...
		private String readString(ObjectInputStream dis) throws IOException {
			...
			if (length == -1) {
				...
				if (length > commonByteArray_AP_UNIX_QA_Framework_Process_CMC.length) {
					if (length < 1024
							&& commonByteArray_AP_UNIX_QA_Framework_Process_CMC.length == 0) {
						commonByteArray_AP_UNIX_QA_Framework_Process_CMC = new byte[1024];
					} else {
						commonByteArray_AP_UNIX_QA_Framework_Process_CMC = new byte[2 * length];
					}
				}
				dis.readFully(commonByteArray_AP_UNIX_QA_Framework_Process_CMC,
						0, length);
				strReturn = new String(
						commonByteArray_AP_UNIX_QA_Framework_Process_CMC, 0,
						length, utf8Charset);
			}
			return strReturn;
		}
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_AP_UNIX_QA_Framework_Process_CMC) {
		...
		try {
			...
			if (resumeEntryMethodName == null || resumeIt || globalResumeTicket) {// start
				...
				log4jParamters_tOracleInput_1
						.append("QUERY"
								+ " = "
								+ "\"  SELECT  SQ.RULE_ID, HEADER_SQL,DATA_SQL,nvl(NOTIFY,'N'),TO_CHAR(EXEC_TIME,'MM/DD/YYYY HH:MI:SS AM'),RULE_DESC,NVL(NOTIFY_SUCCESS,'N'),NVL(NOTIFY_ERROR,'N') FROM QA_SQL SQ  INNER JOIN QA_BUSINESS_RULES BR  	ON SQ.RULE_ID = BR.RULE_ID  WHERE COMP_TYPE = 'CMC'  \"");
				log4jParamters_tOracleInput_1.append(" | ");
	...
	public static void main(String[] args) {
		final QA_Framework_Process_CMC QA_Framework_Process_CMCClass = new QA_Framework_Process_CMC();

		int exitCode = QA_Framework_Process_CMCClass.runJobInTOS(args);
		if (exitCode == 0) {
			log.info("TalendJob: 'QA_Framework_Process_CMC' - Done.");
		}
	...
	public int runJobInTOS(String[] args) {
		...
		log.info("TalendJob: 'QA_Framework_Process_CMC' - Start.");
		...
		try {
			...
			java.io.InputStream inContext = QA_Framework_Process_CMC.class
					.getClassLoader().getResourceAsStream(
							"ap_unix/qa_framework_process_cmc_0_1/contexts/"
			...
			context.successMessage = (String) context
		...
		if (parentContextMap != null && !parentContextMap.isEmpty()) {
			...
			if (parentContextMap.containsKey("successMessage")) {
				context.successMessage = (String) parentContextMap
						.get("successMessage");
			}
		...
		if (false) {
			System.out
					.println((endUsedMemory - startUsedMemory)
							+ " bytes memory increase when running : QA_Framework_Process_CMC");
		}
